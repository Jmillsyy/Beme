 ...
 *
 * When `bodyDepthMm` is provided, only fractions that share the body
 * block's depth survive - a 100-series wall (90mm deep body) won't
 * accept a 200-series 290×190×190 fraction even though both are tagged
 * 'fraction', because the 190-depth piece would poke 100mm out of the
 * 90-deep wall face. Without it, every fraction in the library is
 * returned (back-compat for callers that haven't migrated yet).
 */
    ...
    .filter(
      (b) =>
        ...
        (bodyDepthMm === undefined ||
          Math.abs(b.dimensions.depthMm - bodyDepthMm) < 0.5)
    )
    .sort((a, b) => b.dimensions.widthMm - a.dimensions.widthMm)
}
export function pickFractionBlocks(bodyDepthMm?: number): Block[] {
  return pickFractionBlocksIn(BLOCK_LIBRARY, bodyDepthMm)
}
