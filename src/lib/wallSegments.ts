...
export function resolveWallCourses(
  ...
  let courseNum = 1
  // Standard course module = the body block's height + mortar (AU 190 →
  // 200). Any band whose modular height differs is a HEIGHT-MAKEUP band
  // (20.71 at 100mm modular, 20.140 at 150mm) - those carry their own
  // distinct block code which we MUST preserve in bodyCode so the 3D
  // renders them with their own colour. The series-range body override
  // only applies to standard body courses.
  const stdCourseModuleMm =
    (library[scopedMakeup.bodyBlockCode]?.dimensions.heightMm ?? 190) +
    DEFAULT_MORTAR_JOINT_MM
  ...
  for (const band of bands) {
    ...
    const isHeightMakeupBand =
      Math.abs(bandModuleMm - stdCourseModuleMm) > 0.5
  ...
  const lintelFootprints = disableBlockLintels
    ...
    : wallOpenings
        .map((op) => {
          ...
          // Lintel end-bearing: extend the lintel past each jamb so its
          // ends rest on the masonry either side of the opening. The
          // per-opening override wins over the project default; clamp the
          // span to the wall so a bearing near a corner can't run off the
          // end. Body cells the bearing now covers are cleared by
          // stampZone below and re-counted as lintel, so render == tally.
          const bearingMm =
            sourceOp?.lintelBearingMmOverride ??
            getUserSettings().defaults.defaultLintelBearingMm ??
            0
          const bearingM = Math.max(0, bearingMm) / 1000
          return {
            code,
            spanStart: Math.max(0, op.start - bearingM),
            spanEnd: Math.min(length, op.end + bearingM),
            y0: op.head,
            y1,
