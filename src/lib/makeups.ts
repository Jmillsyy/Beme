...
export function getCourseCount(makeup: WallMakeup): number {
  ...
  if (!bands || bands.length === 0) {
    // Course module = body block height + mortar (AU 190 → 200) so the
    // no-coursePattern count is right for any block height.
    const courseModuleMm =
      (BLOCK_LIBRARY[makeup.bodyBlockCode]?.dimensions.heightMm ?? 190) +
      DEFAULT_MORTAR_JOINT_MM
    return Math.max(0, Math.round(makeup.heightMm / courseModuleMm))
  }
