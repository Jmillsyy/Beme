...
export interface Opening {
  ...
  /**
   * Per-opening lintel end-bearing / overlap (mm), applied to EACH side of
   * the opening head. Overrides the project default
   * (EstimatingDefaults.defaultLintelBearingMm) for THIS opening. The
   * lintel span becomes opening width + 2 × bearing (clamped to the wall),
   * so a 1000mm opening at 190mm bearing lays a 1380mm lintel.
   *
   * Undefined = inherit the project default. 0 = explicitly no bearing on
   * this opening even if the default is non-zero.
   */
  lintelBearingMmOverride?: number
  /**
